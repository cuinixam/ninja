...
`logfile`:: path to an optional file to dump the command's console output into.
  If present, the command's console output (stdout and stderr) will be written
  to this file. If the command's console output is empty, there will be no file
  created. These files are cleaned by default and do not re-trigger the build
  action if they are not present.
