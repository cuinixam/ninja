...
bool ImplicitDepLoader::LoadImplicitOutputs(Edge* edge, string* err) {
  ...
  if (!dynout.empty())
    return LoadOutputsFromLog(edge, err);
...
bool ImplicitDepLoader::LoadOutputsFromLog(Edge* edge, string* err) {
  ...
  DepsLog::Deps* deps = deps_log_ ? deps_log_->GetDeps(output) : NULL;
  if (!deps) {
    EXPLAIN("outputs for '%s' are missing", output->path().c_str());
    return false;
  }

  // Deps are invalid if the output is newer than the deps.
  if (output->mtime() > deps->mtime) {
    EXPLAIN("stored outputs info out of date for '%s' (%" PRId64 " vs %" PRId64
            ")",
            output->path().c_str(), deps->mtime, output->mtime());
    return false;
  }
  ...
  for (size_t i = deps->node_count - deps->outputs_count; i < deps->node_count; i++) {
    ...
    if (!exist) {
      if (!new_node->StatIfNecessary(disk_interface_, err)) {
        return false;
      }
